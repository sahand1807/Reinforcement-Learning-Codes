...
class FootballEnv_v2(gym.Env):
    """
    A standalone, two-phase football environment with enhanced visuals.
    - Phase 1: Agent navigates to the stationary ball.
    - Phase 2: Agent shoots the ball at the goal.
    """
    ...
    def __init__(self, render_mode="human"):
        ...
        self.action_space = self.shooting_action_space # The larger of the two
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Set random ball and goal positions
        ...
        # Set agent's starting position randomly, ensuring it's not on the ball
        while True:
    ...
    def step(self, action):
        ...
        if self.phase == 1:
            ...
            reward = -0.5 # Cost for each step
            if not (0 <= self.agent_x <= 5 and -5 <= self.agent_y <= 5):
                reward, terminated = -10.0, True # Boundary violation
            elif np.allclose([self.agent_x, self.agent_y], [self.ball_x, self.ball_y]):
                reward, self.phase = +10.0, 2 # Reached the ball
            ...
            # Calculate full trajectory for animation
    ...
    def render(self):
        ...
        # Update ball position (stationary in Phase 1, moving in Phase 2 animation)
        ...
        # Update detailed goal visualization
        visual_width = 0.5
    ...
    def animate_shot(self, fps=60):
        ...
        for x, y in zip(self.traj_x, self.traj_y):
            self.traj_x_full.append(x); self.traj_y_full.append(y)
            self.render(); time.sleep(dt)

    def close(self):
        if self.fig is not None: plt.close(self.fig); self.fig = None; plt.ioff()

# =============================================================================
# EXAMPLE USAGE WITH A RANDOM AGENT
# =============================================================================
if __name__ == '__main__':
    ...
    for episode in range(5):
        ...
        while not terminated:
            # Select action based on current phase
            if env.phase == 1:
                action = env.movement_action_space.sample()
            else: # Phase 2
                action = env.shooting_action_space.sample()

            obs, reward, terminated, _, _ = env.step(action)
            
            if env.phase == 1:
                print(f"Move Reward: {reward:.1f}", end='\r')
        ...
        # Post-episode report
        if env.phase == 2:
            print("\nAgent reached ball and shot!")
            print("Animating shot...")
            env.animate_shot()
